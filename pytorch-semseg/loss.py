...
def multi_scale_cross_entropy2d(input, target, weight=None, size_average=True, scale_weight=None):
    ...
    # Auxiliary training for PSPNet [1.0, 0.4] and ICNet [1.0, 0.4, 1.6]
...
def bootstrapped_cross_entropy2d(input, target, K, weight=None, size_average=True):
    ...
    def _bootstrapped_xentropy_single(input, target, K, weight=None, size_average=True):
        n, c, h, w = input.size()
        input = input.transpose(1, 2).transpose(2, 3).contiguous().view(-1, c)
        target = target.view(-1)
        loss = F.cross_entropy(
    ...
    for i in range(batch_size):
        loss += _bootstrapped_xentropy_single(
...
logger = logging.getLogger("Ssgmentation")
...
def get_loss_function(loss):
    if loss is None:
        ...
        if loss not in key2loss:
            raise NotImplementedError("Loss {} implemented".format(loss))
